...
This app utilizes the TicketMaster API. The seeds file runs every artist name from our Artist table through the TicketMaster API search. The TicketMaster API returns the events for each artist and each artist's event's JSON file is parsed so the event data can be utilized. Once the event is in appropriate (database) format, the seed file creates a new entry for each unique Location and Concert. 
